...
void CountSort(int * mas,  int left, int right, int k);
void ModifiedCountSort(int * mas,  int left, int right, int digit);
void RadixSort(int * mas,  int left, int right);




int main() {
	int mas[] = {12,1,5,6,4,2,3};
	//CountSort(mas,0,6, 7);
	RadixSort(mas,0,7);
	//int k = Partition(mas,2,6);
	//printf("%d\n\n",k);
	for (int i =0; i <7; i++) printf("%d ",mas[i]);
    return 0;
}

int kDigitOfNum (int x, int k)
{
	int dlPow10= 10, drPow10 = 1;
	for (int i = 0 ; i < k ; i++) 
	{
		dlPow10*=10;
		drPow10*=10;
	}
	return (x%dlPow10) / drPow10;
}

void RadixSort(int * mas,  int left, int right)
{
	for (int i = 0 ; i <10; i++) ModifiedCountSort(mas,left,right, i);
	return;
...
{
	const int k = 10;
	int C [k]; // int * C = (int * )calloc(k,sizeof(int));
	for (int i =0; i < k; i++) C[i]=0;
	for (int i = left; i<right ; i++) C[kDigitOfNum(mas[i],digit)]++; // Подсчет
	for (int i =1; i < k; i++) C[i]+= C[i-1]; // Это С с волной
	int ans [right-left]; // int * ans = (int * )malloc((right-left)*sizeof(int));
	for (int i = right-1; i>=left ; i--)
		ans[--C[kDigitOfNum(mas[i],digit)]] = mas[i];
	for (int i = left; i<right ; i++) mas[i]=ans[i-left];
	return;	
}
void CountSort(int * mas,  int left, int right, int k)
{
	int C [k]; // int * C = (int * )calloc(k,sizeof(int));
	for (int i =0; i < k; i++) C[i]=0;
	for (int i = left; i<right ; i++) C[mas[i]]++; // Подсчет
	for (int i =1; i < k; i++) C[i]+= C[i-1]; // Это С с волной
	int ans [right-left]; // int * ans = (int * )malloc((right-left)*sizeof(int));
	for (int i = right-1; i>=left ; i--)
		ans[--C[mas[i]]] = mas[i];
...
int Partition(int * mas, int left, int right) // возврат индекса по которому проходит граница
{
	...
	// int pivot = mas[rand()% (right - left) + left]; 
